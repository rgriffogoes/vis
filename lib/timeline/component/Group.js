...
Group.prototype.redraw = function(range, margin, forceRestack) {
  ...
  if(restack) {
    if (typeof this.itemSet.options.order === 'function') {
      ...
      if (this.itemSet.options.hideOverlapping) { // TODO: ugly way to access options...
        this._hideOverlapping(this.orderedItems.byStart);
      }
...
/**
 * Hides overlapping items
 */
Group.prototype._hideOverlapping = function(orderedItems) {
  // reset overlapping items 
  for (var i = 0; i < orderedItems.length; i++) {
    orderedItems[i].previousOverlapCount = (orderedItems[i].overlaps)? orderedItems[i].overlaps.length : 0;
    orderedItems[i].overlaps = [];
    orderedItems[i].overlapHidden = false;
  }
  var lastCollision = 0;
  var range = this.itemSet.body.range; // Not a nice way to get range!
  var visibleItems = [];
	
  // calculate new, non-overlapping positions
  for (var i = 0; i < orderedItems.length; i++) {
    var item = orderedItems[i];
		if (!item.isVisible(range) || !item.collisionOverlap || (!item.calculateX &&  !item.dom)) continue; 
    
		//if implements dom'less calculateX, use it. Otherwise, use default repositionX
		if (item.calculateX) item.calculateX(); else item.repositionX(); 
	
    // TODO: optimize checking for overlap. when there is a gap without items,
    //       you only need to check for items from the next item on, not from zero
    var collidingItem = null;
    for (var j = lastCollision; j < i; j++) {
      var other = orderedItems[j];
	    if (!other.isVisible(range) || other.overlapHidden || !other.collisionOverlap || (!other.calculateX &&  !other.dom)) continue;
	  
      if (this._hideOverlappingColllision(item, other, item.collisionOverlap(), other.options.rtl, 8)) {
        // There is a collision.
        collidingItem = other;
        lastCollision = j;
        collidingItem.overlaps.push(item);
        item.hide();
        item.overlapHidden = true;
        break;
      }
    }
		if (!item.overlapHidden) visibleItems.push(item);
  }
	// redrawing items that changed number of overlaps
...
/**
 * Checks for colision between two items which implements the DOM-independent calculateX methods.
 * @param {{a: item, b: item}} entities to check colision - must implement the calculateX method
 * @param {number} overlap - number of overlapping pixels to consider as colision
 * @param {boolean} rtl
 * @param {number} defaultWidth - default item width
 * @return {boolean} Returns true if the colision is larger than overlap
 */
Group.prototype._hideOverlappingColllision = function(a, b, overlap, rtl, defaultWidth) {
  if ((!a.calculateX && !a.dom) || (!b.calculateX && !b.dom)) return false; 
  
  var aWidth = (a.props && a.props.dot && a.props.dot.width ? a.props.dot.width : a.width || defaultWidth);
  var bWidth = (b.props && b.props.dot && b.props.dot.width ? b.props.dot.width : b.width || defaultWidth);
  
  var min1,min2,max1,max2;
  if (rtl) {
    min1 = a.right; max1 = a.right + aWidth;
    min2 = b.right; max2 = b.right + aWidth;
  } else {
    min1 = a.left; max1 = a.left + aWidth;
    min2 = b.left; max2 = b.left + bWidth;
  }
  return Math.ceil(Math.max(0, Math.min(max1, max2) - Math.max(min1, min2))) >= Math.min(overlap,bWidth);
};
...
Group.prototype._checkIfVisible = function(item, visibleItems, range) {
    if (!this.itemSet.options.hideOverlapping && item.isVisible(range)) {
      if (!item.displayed) item.show();
      // reposition item horizontally
      item.repositionX();
      visibleItems.push(item);
