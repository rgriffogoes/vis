...
ItemSet.prototype._create = function(){
  ...
  this.body.dom.centerContainer.addEventListener('click', this._onMouseClick.bind(this));
...
ItemSet.prototype.setOptions = function(options) {
  if (options) {
    ...
    var fields = [
      ...
      'hide', 'hideOverlapping', 'snap', 'groupOrderSwap', 'showTooltips', 'tooltip', 'tooltipOnItemUpdateTime'
    ];
...
ItemSet.prototype.redraw = function() {
  ...
  /// TODO improve and redraw/reposition tooltip if possible
  if (forceRestack && this.popup != null) {
    this.stickyPopup = false;
    this.popup.hide();
      // console.log("hiding in redraw"+(new Date().getTime()));
  }
...
/**
 * Handle click an item (sticky tooltip)
 * @param {Event} event
 * @private
 */
ItemSet.prototype._onMouseClick = function (event) {
    var item = this.itemFromTarget(event);
    if (!item)
    {
        this.stickyPopup = false;
        if (this.popup != null) this.popup.hide();
        // console.log("hiding in click1"+(new Date().getTime()));
        return;
    }
    ...
    if (this.options.showTooltips && title) {
        if (this.popup == null) {
            ...
            this.popup.frame.addEventListener('click', function(e){
                e.preventDefault();
                e.stopPropagation();
                return false;
            });
        ...
        this.popup.frame['timeline-item'] = item;
        this.popup.frame.item = item;
        this.popup.show();
        this.stickyPopup = true;
    } else {
        ...
        if (this.popup != null) {
            this.stickyPopup = false;
            this.popup.hide();
            // console.log("hiding in click2"+(new Date().getTime()));
        }
    }
};
/**
...
ItemSet.prototype._onMouseOver = function (event) {
  if (this.stickyPopup) return;
  var item = this.itemFromTarget(event);
  if (!item) {
    if (this.popup != null && !this.popup.hidden) {
        this.popup.hide();
        // console.log("hiding in mouseover1"+(new Date().getTime()));
    }
    return;
  }
  ...
  if (this.options.showTooltips && title) {
    if (this.popup == null) {
      this.popup = new Popup(this.body.dom.root,
          this.options.tooltip.overflowMethod || 'flip');
      this.popup.frame.addEventListener('click', function(e){
          // console.log("click on tooltip from item: ", this.item);
          e.preventDefault();
          e.stopPropagation();
          return false;
      });
    ...
    this.popup.setPosition(
      event.clientX - util.getAbsoluteLeft(container) + container.offsetLeft,
      // event.clientY - util.getAbsoluteTop(container) + container.offsetTop
        item.parent.dom.foreground.offsetTop + item.parent.dom.foreground.offsetHeight/2
    );
    ...
    if (this.popup != null) {
      this.popup.hide();
        // console.log("hiding in mouseover"+(new Date().getTime()));
    }
  }
...
ItemSet.prototype._onMouseOut = function (event) {
  var item = this.itemFromTarget(event);
  if (!item || this.stickyPopup) return;
  ...
  if (this.popup != null) {
    // console.log("hiding in mouseout"+(new Date().getTime()), item, related );
    this.popup.hide();
  }
